...
class CustomCanvas:
    ...
    def __init__(self, width, height, center, refresh_rate, cursor, events=None):
...
def cree_fenetre(largeur, hauteur, center="no", frequence=100, cursor=''):
    """
    ...
    __canevas = CustomCanvas(largeur, hauteur, center, frequence, cursor)
...
def set_resizeable(resizeable):
    """
    Prevent or allow the windows to be resized
    """
    if __canevas is None:
        raise FenetreNonCree(
            "La fenêtre n'a pas été crée avec la fonction \"cree_fenetre\".")
    __canevas.root.resizable(resizeable, resizeable)
